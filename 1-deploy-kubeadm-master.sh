...
multipass exec master -- bash -c 'sudo curl -fsSL https://get.docker.com -o get-docker.sh'
multipass exec master -- bash -c 'sudo sh get-docker.sh'
# Setup daemon.
...
multipass exec master -- bash -c 'sudo apt-get install -y kubelet=1.19.2-00 kubeadm=1.19.2-00 kubectl=1.19.2-00'
...
multipass exec master -- bash -c 'sudo kubeadm init --kubernetes-version=v1.19.2 --pod-network-cidr=192.178.0.0/16'
...
echo "Enjoy the kubeadm made Kubernetes 1.19.x on Multipass Ubuntu 18:04"
